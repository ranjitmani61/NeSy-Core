...
from nesy.neural.loss import LukasiewiczLogic, SymbolicHingeLoss
...
class TestContinualLearningFull:
    ...
    def test_two_tasks_no_forgetting(self):
        ...
        # Task A
        model.learn(SymbolicRule("task_a", [Predicate("A", ("?x",))],
                                 [Predicate("B", ("?x",))], 0.9))
        output_a1 = model.reason({Predicate("A", ("x1",))})
        assert "B" in output_a1.answer

        # Task B
        model.learn(SymbolicRule("task_b", [Predicate("C", ("?x",))],
                                 [Predicate("D", ("?x",))], 0.9))
    ...
    def test_anchor_cannot_be_overwritten(self):
        model = NeSyModel()
        model.learn(SymbolicRule("permanent", [Predicate("X", ("?a",))],
                                 [Predicate("Y", ("?a",))], 1.0), make_anchor=True)
        with pytest.raises(Exception):
            model.learn(SymbolicRule("permanent", [Predicate("X", ("?a",))],
                                     [Predicate("Z", ("?a",))], 1.0), make_anchor=True)

    def test_anchor_rule_count_increases(self):
        model = NeSyModel()
        model.learn(SymbolicRule("anc1", [Predicate("A", ("?x",))],
                                 [Predicate("B", ("?x",))], 1.0), make_anchor=True)
        model.learn(SymbolicRule("anc2", [Predicate("C", ("?x",))],
                                 [Predicate("D", ("?x",))], 1.0), make_anchor=True)
...
class TestReplayStrategies:
    ...
    def test_symbolic_anchor_replay(self):
        ...
        rule   = SymbolicRule("r1", [Predicate("A", ("?x",))],
                              [Predicate("B", ("?x",))], 1.0)
        anchor.add(rule)
