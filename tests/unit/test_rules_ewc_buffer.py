...
class TestRuleBuilder:
    ...
    def test_anchor_sets_immutable(self):
        rule = RuleBuilder("anchor_rule").if_fact("X", "?a").then("Y", "?a").as_anchor().build()
...
def mortal_rules():
    return [
        SymbolicRule(
            "human",
            [Predicate("IsPhilosopher", ("?x",))],
            [Predicate("IsHuman", ("?x",))],
            weight=1.0,
        ),
        SymbolicRule(
            "mortal", [Predicate("IsHuman", ("?x",))], [Predicate("IsMortal", ("?x",))], weight=1.0
        ),
    ]
